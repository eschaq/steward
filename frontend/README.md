...
| `public/brand/`               | Hero photography and the sign-in clip (greyscale; duotoned in CSS) |
...
`public/brand/hero-drift-loop.mp4` — 6.5s, 1280×720, 2.0 MB, generated by
`scripts/generate_hero_video.py --no-seed` (Veo 3.1 Fast on Vertex AI) and
re-cut with ffmpeg into a crossfaded loop. **Nothing in the running product
calls Veo**; this is a build asset. The ungraded 8-second master is kept in
`brand-source/`, outside `public/`, so it is never served or shipped.

A slow dolly past the gable end of a stone cottage — weathered oak apex,
hand-cut terracotta stone, slate roof, late golden light — with real parallax as
the roof edge travels across the stonework.

**Generated from text, not seeded from the still.** Seeding Veo with
`hero-gable-landscape.jpg` produced a near-static clip: it re-lit the photograph
rather than moving through it, and measured an edge difference of ~10/255 across
eight seconds, essentially all of it a global brightness ramp. Generating from
the prompt alone measured **~64**. The trade is that the building is not the one
in the stills, which is why the sign-in screen's poster and background-image are
now frames from *this* clip — a failed video falls back to the same house. The
`.hero` app bar keeps the original gable and is unchanged.

**A travelling shot does not loop by trimming**, because the camera ends
somewhere else: a hard cut measured an edge difference of 64 end-to-end. The clip
is re-cut as an overlap crossfade — the last 1.5s dissolving into the first — so
the loop closes as a soft transition rather than a jump.

**Autoplay needs the `muted` *attribute*, and React doesn't write one.** React
applies `muted` as a DOM property only, while Chrome reads the attribute as the
element is parsed to decide whether autoplay is allowed — so `<video muted
autoPlay>` in JSX reaches the browser looking unmuted and gets blocked. It
loads, paints its poster, and sits there. A `useEffect` sets
`muted` / `defaultMuted` / `setAttribute('muted','')` and then calls `play()`,
with one retry on the first pointer or key event.

**The `?v=` token on the src is a cache-buster and it earns its keep.** This file
was replaced in place twice during development and Chrome kept serving the old
one — a range-requested video survives an ordinary hard refresh. Bump the token
whenever the clip is regenerated or re-cut.

Verified:

| Condition | Result |
| --- | --- |
| Default policy | plays, muted attribute present, 6.5s |
| Strict autoplay policy | plays |
| `.mp4` blocked | falls back to `hero-drift-poster.jpg` — the same house — form intact, no layout shift |
| `prefers-reduced-motion: reduce` | `display: none`, poster shows through |

Four generations, ~$3.20 at list price: one seeded, one seeded with a push-in
(no better — 10.37 against 10.29, which is what settled that prompt-wrangling
was not the lever), one text-only, and the crossfade re-cut was free.
