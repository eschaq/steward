...
ask_about_unclear_item_tool = FunctionTool(ask_about_unclear_item)
mediate_contested_item_tool = FunctionTool(mediate_contested_item)

# Which behavior belongs to which state. The backend detects the transition; this
# is the mapping, not a judgement call.
TOOL_FOR_STATUS = {
    ItemStatus.NEEDS_CLARIFICATION: ask_about_unclear_item_tool,
    ItemStatus.CONTESTED: mediate_contested_item_tool,
}

steward_agent = LlmAgent(
    model=AGENT_MODEL,
    name="steward",
    ...
    tools=[ask_about_unclear_item_tool, mediate_contested_item_tool],
)


async def invoke_tool(tool: FunctionTool, **args: Any) -> dict[str, Any]:
    ...
    return await tool.run_async(args=args, tool_context=ToolContext(context))
...
async def run_behavior_for_item(item_id: str) -> dict[str, Any]:
    ...
    attaches to — better than a 200 that quietly did nothing.
    """
    item = _load_item(item_id)
    tool = TOOL_FOR_STATUS.get(item.status)
    if tool is None:
        attached = ", ".join(sorted(s.value for s in TOOL_FOR_STATUS))
        raise AgentError(
